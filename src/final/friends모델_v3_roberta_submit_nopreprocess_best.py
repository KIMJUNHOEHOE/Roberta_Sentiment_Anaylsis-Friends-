# -*- coding: utf-8 -*-
"""Friends모델_v3_roberta_submit_nopreprocess.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1xYsg5Mn3jTPHuxvc8EXVsJ6DyVID0wzZ
...
# 훈련셋 / 테스트셋 로드
submit_data = pd.read_csv("drive/MyDrive/en_data.csv",sep=',')
submit_data1 = pd.read_csv("drive/MyDrive/en_data.csv",sep=',')
...
model.load_state_dict(torch.load(PATH + 'Friends_roberta_state_dict_v4_best.pt')) ## 파일명
...
for rows in submit_data['utterance']:
  data_submit.append([preprocess(rows)])

# 훈련셋 / 테스트셋 로드
data_submit = pd.DataFrame(data_submit, columns=['document'])

data_submit

# 문장, label 추출
sen_submit = data_submit['document']
...
tokenizer = RobertaTokenizer.from_pretrained('roberta-base', do_lower_case=False) # wordpiece tokenizer 사용
...
# 문장을 MAX_LEN 길이에 맞게 자르고, 모자란 부분을 패딩 0으로 채움
input_ids_submit = pad_sequences(input_ids_submit, maxlen=MAX_LEN, dtype="long", truncating="post", padding="post", value=1.0)
...
label_list = [dic_label[i] for i in predicted_label]

label = pd.DataFrame(label_list,columns=['Predicted'])

submit_data

label

submit = pd.concat([submit_data1, label],axis=1)

submit = submit.rename(columns = {'id' : 'Id'})[['Id','Predicted']]
submit

submit['Predicted'].unique()

submit.to_csv('drive/MyDrive/Friends_roberta_v4_ep4_best.csv',
              sep=',',
              index = False)
